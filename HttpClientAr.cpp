...
//
//HTTP WIFI CLIENT
HttpWifiClientAr::HttpWifiClientAr(): WiFiEspClient(){}

HttpResponse HttpWifiClientAr::httpResponse(){
  HttpResponse httpResponse;
  String header= this->readHeader();
  httpResponse.setVarsHeader(header);
  httpResponse.setBody(this->readBody());  
...
String HttpWifiClientAr::readHeader(){
  String header;
  //an http request ends with a blank line
  boolean currentLineIsBlank = true;
  while (this->connected()) {
...
String HttpWifiClientAr::readBody(){
  String body;
  boolean currentLineIsBlank = true;
  while (this->connected()) {
    if (this->available()) {
...
void HttpWifiClientAr::sendRequest(String method, String uri){
  this->println(method + " " + uri + " HTTP/1.1");
  this->println("Host: 10.10.10.100");  
  this->println("User-Agent: arduino-wifi");  
  this->println("Connection: close");
  this->println();
}
void HttpWifiClientAr::sendRequest(String method, String uri, String contentType){
  this->println(method + " " + uri + " HTTP/1.1");
  this->println("Host: 10.10.10.100");  
  this->println("User-Agent: arduino-wifi");  
  this->println("Connection: close");
...
void HttpWifiClientAr::sendRequest(String method, String uri, String contentType, String &body){
  this->println(method + " " + uri + " HTTP/1.1");
  this->println("Host: 10.10.10.100");  
  this->println("User-Agent: arduino-wifi");  
  this->println("Connection: close");
...
void HttpWifiClientAr::sendBody(String &body){
  this->println(body);
}
